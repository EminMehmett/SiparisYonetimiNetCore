    ...
    public class ProductsController : Controller
    {
        ...
        public async Task<ActionResult> DeleteAsync(int id, Product product)
        {
            try
            {
                ...
                var response = await _httpClient.DeleteAsync($"{_apiAdres}Products/{id}"); // response = cevap
                if (response.IsSuccessStatusCode) FileHelper.FileRemover(product.Image); // IsSuccess = başarılıysa
